// static/js/travel-advisories.js - VERSION CORRIGÉE
class TravelAdvisoriesManager {
    static async initialize() {
        ...
        try {
            ...
            console.error('❌ Erreur initialisation avis voyageurs:', error);
        }
    }
    ...
    static async loadAdvisories() {
        try {
            ...
            if (data.success) {
                this.displayAdvisories(data.countries);
                this.updateStats(data.countries);

                // Afficher la source des données
                if (data.source === 'mock') {
                    console.log('ℹ️ Utilisation données mockées pour travel advisories');
                }
            } else {
